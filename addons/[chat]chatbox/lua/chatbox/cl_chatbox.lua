...
File Path:   addons/[chat]_sh_lounge_chatbox/lua/chatbox/cl_chatbox.lua
    ...
    if (IsValid(ply)) then
        if ply:GTeam() != TEAM_SPEC then
            if render_character_avatar:GetBool() then
                ...
                if ( ( LocalPlayer():CanSeeEnt(ply) and !ply:GetNoDraw() ) or ply == LocalPlayer() or ( ply:GTeam() == TEAM_SCP and LocalPlayer():GTeam() == TEAM_SCP ) ) then
