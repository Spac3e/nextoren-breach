...
function DiscordUpdate()
    ...
    rpc_data["state"] = "Локация: "..MP_ICONS[game.GetMap()].largeText

    for i, ply in ipairs(player.GetAll()) do
        if ply:GetNamesurvivor() == "none" or !ply:GetNamesurvivor() then
            local faggot = LocalPlayer()
            rpc_data["details"] = "Роль: "..faggot:GetRoleName()
            rpc_data["smallImageKey"] = team_iconka[gteams.GetName(faggot:GTeam())]
