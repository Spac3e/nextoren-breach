local MenuTable = {}
MenuTable.url = "https://vault-breach.fandom.com/ru/wiki/"
...
local spac = "-Spac3"
local ipik = "Imperator"
local ous = "-Spac3&Imperator"
local vault = "VAULT Contributors/Team"

local PATCHIE = {
  {type = "title", str = "22/06/2023"},
  {str = "Переход функций работы раунда с клиента на сервер", by = spac},
  {str = "Заменил использование старых функицй, теперь для звуков есть net-ы\n и функции с серверной части", by = spac},
  {str = "Пофиксил МОГ", by = spac},
  {type = "title", str = "21/06/2023"},
    {str = "Добавлен Bonemerge", by = ous},
    {str = "Настроены роли и их бодигруппы", by = spac},
    {str = "Обновления для админского ULX", by = spac},
    {str = "Фикс PhysBullet", by = spac},
    {str = "Фикс CW 2.0 Оружий", by = spac},
    {str = "Добавлено имя для персонажа", by = spac},
    {str = "Обновлено MainMenu", by = spac},
    {str = "Добавлены функции для администрации и премиумов", by = spac},
}
...
mainmenumusic:Play()
  local buttons = {
    ...
    },
    {
      name = "rules",
      func = function()
        INTRO_PANEL:OpenUrl("https://book.vaultcommunity.net/books/pravila-garrys-mod/page/pravila-servera-breach")
        surface.PlaySound( "nextoren/gui/main_menu/main_menu_select_1.wav" )
      end,
    },
    --[[
  ...
  for k, v in ipairs(file.Find("!vault*.dem", "MOD")) do
    --print(v)
  ...
  INTRO_PANEL.OverrideDraw.Paint = function(self, w, h)
    ...
    --draw.DrawText("VAULT", "ScoreboardHeader", 105, 1+offset_height)
    draw.DrawText("VAULT Breach 2.6.0", "ScoreboardHeader", 105, 25+offset_height)
    surface.SetDrawColor(color_white)
    ...
    draw.DrawText("Created by -Spac3 | Imperator | Maleyvich", "ScoreboardContent", 105, 65)
  ...
  local spisok = {
    {defaultname = "-Spac3", id = "76561198797549224", who = BREACH.TranslateString"l:menu_lead l:menu_dev"},
    {defaultname = "Imperator", id = "76561198966614836", who = BREACH.TranslateString"l:menu_lead l:menu_dev"},
    {defaultname = "Maleyvich", id = "76561198170506041", who = BREACH.TranslateString"l:menu_maleyvich"},
  }
...
function send_prefix_data()
  ...
  local data = util.JSONToTable(file.Read("vault_dick.txt", "DATA"))
...
hook.Add("InitPostEntity", "StartBreachIntro", function()
  StartBreach(true)
  if !file.Exists("vault_dick.txt", "DATA") then
    file.Write("vault_dick.txt", util.TableToJSON({
      enabled = false,
      prefix = "my prefix",
      color = "255,255,255",
      rainbow = false,
...
CreateConVar("breach_config_display_weapon_info", 0, FCVAR_ARCHIVE, "Disables role description", 0, 1)
...
function OpenDonateMenu()
  ...
  panel.Paint = function(self, w, h)
    ...
    draw.DrawText("По вопросам пишите:\n-Spac3#5208", "ScoreboardHeader", w/2, 5, Color(255,0,0), TEXT_ALIGN_CENTER)
  end
...
function OpenConfigMenu()
  ...
  local function create_prefix_panel(data)
    ...
    apply.DoClick = function()
      ...
      file.Write("vault_dick.txt", util.TableToJSON(data, true))
    ...
    for _, data in pairs(category.settings) do
      ...
      elseif data.type == "prefix" then
        ...
        edit.DoClick = function()
          create_prefix_panel(util.JSONToTable(file.Read("vault_dick.txt", "DATA")))
        end
