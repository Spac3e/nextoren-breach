RESEARCHERMODELS = {
	"models/cultist/humans/sci/scientist.mdl",
	"models/cultist/humans/sci/scientist_female.mdl",
	}
	SPECIAL1 = {
	"models/cultist/humans/scp_special_scp/special_1.mdl"
	}
	SPECIAL2 = {
	"models/cultist/humans/scp_special_scp/special_2.mdl"
	}
	SPECIAL3 = {
	"models/cultist/humans/scp_special_scp/special_3.mdl"
	}
	SPECIAL4 = {
	"models/cultist/humans/scp_special_scp/special_4.mdl"
	}
	SPECIAL5 = {
	"models/cultist/humans/scp_special_scp/special_5.mdl"
	}
	SPECIAL6 = {
	"models/cultist/humans/scp_special_scp/special_6.mdl"
	}
	SPECIAL7 = {
	"models/cultist/humans/scp_special_scp/special_7.mdl"
	}		
	NTFMODELS = {
	"models/cultist/humans/ntf/ntf.mdl"
	}
	CHAOSMODELS = {
	"models/cultist/humans/chaos/chaos.mdl"
	}
	GOCMODELS = {
	"models/cultist/humans/goc/goc.mdl"
	}
	USAMODELS = {
	"models/cultist/humans/fbi/fbi.mdl"
	}
	DZMODELS = {
	"models/cultist/humans/dz/dz.mdl"		
	}
	SECURITYMODELS = {
	"models/cultist/humans/security/security.mdl"
	}
	GK = {
	"models/cultist/humans/mog/head_site.mdl"
	}
	JAG = {
	"models/cultist/humans/mog/mog_jagger.mdl"
	}
	CHLENIST = {
	"models/cultist/humans/mog/mog_hazmat.mdl"
	}
	MTFMODEL = {
	"models/cultist/humans/mog/mog.mdl"
	}
	HEADPERSONNEL = {
	"models/cultist/humans/sci/scientist_boss.mdl"
	}
	TOPKEK = {
	"models/cultist/humans/class_d/class_d_bor_new.mdl"
	}	
	TOPFAT = {
	"models/cultist/humans/class_d/class_d_fat_new.mdl"
	}	
	TOPHACKER = {
	"models/cultist/humans/class_d/class_d_hacker_new.mdl"	
	}	
	CLASSDMODELS = {
	"models/cultist/humans/class_d/class_d.mdl",
	"models/cultist/humans/class_d/class_d_female.mdl",
	}		
	CLASSDMODELSMALES = {
	"models/cultist/humans/class_d/class_d.mdl",
	}
	
	ALLCLASSES = {
			
				classds = {
			
					name = "Класс Д персонал",
			
					color = Color(255, 130, 0),
			
					roles = {
			
						{name = ROLES.ROLE_CLASSD,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"br_holster"},
			
						 showweapons = {},
			
						ammo = {},
			
						 health = 110,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
			
						 rsdm = true,
		
						 level = 0,
			
						stamina = 2,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 64
			
						},
			
						{name = ROLES.ROLE_VOR,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"br_holster"},
			
						 showweapons = {},
			
						 ammo = {},
			
						 health = 110,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
			
						 level = 20,
		
						 rsdm = true,
			
						 customcheck = nil,
			
						 flashlight = false,
			 
						 ability = {"Thief", 45, "Украдите у человека предмет который он держит на данный момент!", "nextoren/gui/special_abilities/ability_placeholder.png", false},
			
						 max = 3
			
						},
			
						{name = ROLES.ROLE_GOCSPY,
			
						 team = TEAM_GOC,
			
						 weapons = {"br_holster", "breach_keycard_2", "item_knife"},
			
						 showweapons = {"Заточка", "Ключ-Карта Общего Доступа 2"},
			
						ammo = {},
			
						 health = 200,
			
						 armor = 0,
	
						 stamina = 1.5,
			
						 walkspeed = 1,
			
						 runspeed = 1.1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = CLASSDMODELSMALES,
			
						 showmodel = nil,
			
						 level = 18,
			
						 customcheck = nil,
			
						 flashlight = false,
		
						 rsdm = true,
			
						 max = 2
			
						},
			
						{name = ROLES.ClassD_Probitiy,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"br_holster"},
			
						 showweapons = {""},
			
						 ammo = {},
			
						 health = 150,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 1,
			
						 jumppower = 1,
	
						 maxslots = 9,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
		
						 rsdm = true,
			
						 level = 7,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 3
			
						},
						{name = ROLES.ClassD_Pron,
			
						team = TEAM_CLASSD,
		   
						weapons = {"br_holster", "item_screwdriver", "breach_keycard_1"},
		   
						showweapons = {"Ключ карта 1 уровня общего доступа"},
		   
						ammo = {},
		   
						health = 100,
		   
						armor = 0,
		   
						walkspeed = 1,
		   
						runspeed = 1,
		   
						jumppower = 1,
		   
						vest = nil,
		   
						models = CLASSDMODELS,
		   
						showmodel = nil,
		
						rsdm = true,
		   
						level = 14,
		   
						customcheck = nil,
		   
						flashlight = false,
		   
						max = 2
		   
					   },
			
						{name = ROLES.ROLE_Killer,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"item_knife"},
			
						 showweapons = {"Заточка"},
			
						ammo = {},
			
						 health = 110,
			
						 armor = 0,
			
						 walkspeed = 1.1,
			
						 weapons = {"br_holster", "item_knife"},
			
						 runspeed = 1.1,
			
						 jumppower = 1.1,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
		
						 rsdm = true,
			
						 level = 9,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 2
			
						},
			
						{name = ROLES.ROLE_hacker,
			
						 team = TEAM_CLASSD,
			
						 weapons = {},
			
						 showweapons = {"Устройства для взлома"},
			
						ammo = {},
			
						 health = 110,
	
						 stamina = .6,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 weapons = {"br_holster", "hacking_doors"},
			
						 runspeed = 1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = TOPHACKER,
			
						 showmodel = nil,
			
						 level = 4,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 1
			
						},
			
						{name = ROLES.ROLE_Can,
			
						 team = TEAM_CLASSD,
			
						 weapons = {},
			
						showweapons = {"Каннибализм"},
			
						ammo = {},
			
						 health = 90,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 weapons = {"br_holster", "weapon_cannibal"},
			
						 runspeed = 1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
			
						 level = 12,
			
						 customcheck = nil,
			
						 flashlight = false,
		
						 rsdm = true,
			
						 max = 2
			
						},
			
						{name = ROLES.ROLE_Sport,
			
						 team = TEAM_CLASSD,
			
						 weapons = {},
			
						 showweapons = {},
			
						ammo = {},
			
						 health = 120,
			
						 armor = 0,
			
						 walkspeed = 1.10,
						 
						 stamina = 1.6,
			
						 weapons = {"br_holster"},
			
						 runspeed = 1.30,
			
						 jumppower = 1.5,
			
						 vest = nil,
			
						 models = CLASSDMODELS,
			
						 showmodel = nil,
			
						 level = 21,
			
						 customcheck = nil,
			
						 flashlight = false,
		
						 rsdm = true,
			
						 max = 4
			
						},
			
						{name = ROLES.ClassD_Bor,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"br_holster"},
			
						 showweapons = {},
			
						ammo = {},
			
						 health = 220,
			
						 armor = 0,
			
						 walkspeed = 1,
	
						 stamina = 1.5,
			
						 weapons = {"br_holster"},
			
						 runspeed = 0.95,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = TOPKEK,
			
						 showmodel = nil,
			
						 level = 5,
			
						 customcheck = nil,
			
						 flashlight = false,
		
						 ability = {"Бросок", 45, "Вы кидаете на прогиб человека напротив вас.", "nextoren/gui/special_abilities/special_bor_takedown.png", false},
			
						 max = 2
			
						},
			
						{name = ROLES.ROLE_FAT,
			
						 team = TEAM_CLASSD,
			
			
						 showweapons = {"Гамбургер"},
			
						   ammo = {},
			
						 health = 280,
			
						 armor = 0,
			
						 walkspeed = 0.86,
			
						 weapons = {"br_holster", "item_hamburger"},
			
						 runspeed = 0.90,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = TOPFAT,
			
						 showmodel = nil,
			
						 level = 2,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 4
			
				 },
			
						{name = ROLES.ROLE_FARTINHALER,
			
						 team = TEAM_CLASSD,
			
						 weapons = {"br_holster", "item_knife", "breach_keycard_guard_2"},
			
						 showweapons = {},
			
						 ammo = {},
			
						 health = 200,
			
						 armor = 100,
			
						 walkspeed = 1.12,
			
						 runspeed = 1.3,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = CLASSDMODELSMALES,
			
						 showmodel = nil,
			
						 level = 150,
			
						 customcheck = nil,
			
						 flashlight = false,
			
						 max = 1
			
						},
			
					}
			
				},
			
				researchers = {
					name = "Персонал",
					color = Color(66, 188, 244),
					roles = {
						{name = ROLES.ROLE_SCI_ASSISTANT,
						 team = TEAM_SCI,
						 weapons = {"br_holster", "weapon_pass_sci"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headf = {"models/cultist/heads/female/head_1.mdl"},
						 tasks = "Побег из комплекса",
						 keycard = "sci_1",
						 ammo = {},
						 health = 100,
						 armor = 0,
						 walkspeed = 1,
						 recoilmultiplier = 6,
						 runspeed = 1,
						 jumppower = 1,
						 genders = {true, false},
						 hairf = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 models = {"models/cultist/humans/sci/scientist.mdl"},
						 fmodels = {"models/cultist/humans/sci/scientist_female.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist.mdl",
						 level = 0,
						 bodygroup5 = 1,
						 max = 5,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						{name = ROLES.ROLE_SCI_CLEANER,
						 team = TEAM_SCI,
						 weapons = {"br_holster"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 hackerhat = "models/shaky/humans/class_d/head_gear/cleaner_cap.mdl",
						 tasks = "Побег из комплекса",
						 keycard = "1",
						 ammo = {},
						 health = 125,
						 recoilmultiplier = 3.4,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/class_d/class_d_cleaner.mdl"},
						 showmodel = "models/cultist/humans/class_d/class_d_cleaner.mdl",
						 level = 2,
						 customcheck = nil,
						 max = 3,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						{name = ROLES.ROLE_SCI_RES,
						 team = TEAM_SCI,
						 weapons = {"br_holster", "weapon_pass_sci"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headf = {"models/cultist/heads/female/head_1.mdl"},
						 tasks = "Побег из комплекса",
						 keycard = "sci_2",
						 stamina = .5,
						 ammo = {},
						 health = 100,
						 recoilmultiplier = 6,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 genders = {true, false},
						 hairf = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 models = {"models/cultist/humans/sci/scientist.mdl"},
						 fmodels = {"models/cultist/humans/sci/scientist_female.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist.mdl",
						 level = 12,
						 bodygroup2 = 1,
						 max = 4,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						{name = ROLES.ROLE_DZ_SPY,
						 team = TEAM_DZ,
						 weapons = {"br_holster", "weapon_pass_sci"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headf = {"models/cultist/heads/female/head_1.mdl"},
						 tasks = "Побег из комплекса",
						 keycard = "sci_2",
						 ammo = {},
						 health = 110,
						 stamina = 1.5,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 bodygroup2 = 1,
						 genders = {true, false},
						 hairf = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 models = {"models/cultist/humans/sci/scientist.mdl"},
						 fmodels = {"models/cultist/humans/sci/scientist_female.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist.mdl",
						 level = 20,
						 max = 1,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						{name = ROLES.ROLE_SCI_TESTER,
						 team = TEAM_SCI,
						 weapons = {"br_holster", "weapon_pass_sci"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headf = {"models/cultist/heads/female/head_1.mdl"},
						 tasks = "Побег из комплекса",
						 bodygroup1 = 2,
						 bodygroup2 = 1,
						 keycard = "sci_2",
						 stamina = .6,
						 ammo = {},
						 health = 100,
						 recoilmultiplier = 3,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 genders = {true, false},
						 hairf = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 models = {"models/cultist/humans/sci/scientist.mdl"},
						 fmodels = {"models/cultist/humans/sci/scientist_female.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist.mdl",
						 level = 18,
						 max = 2,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						/*
						{["name"] = role.SCI_Recruiter,
						 ["team"] = TEAM_SCI,
						 ["weapons"] = {"br_holster", "weapon_pass_sci"},
						 ["doctype"] = 10,
						 ["head"] = "models/cultist/heads/male/head_main_1.mdl",
						 ["headf"] = {"models/cultist/heads/female/head_1.mdl"},
						 ["ability"] = {"Recruit", 5, "Превращение Класса-Д в научный персонал", "nextoren/gui/special_abilities/ability_recruiter.png", true},
						 ["ability_max"] = 1,
						 ["tasks"] = "Побег из комплекса, пометка Класса-Д",
						 ["bodygroup0"] = 2,
						 ["bodygroup1"] = 0,
						 ["bodygroup2"] = 1,
						 ["bodygroup3"] = 0,
						 ["bodygroup4"] = 1,
						 ["keycard"] = "sci_3",
						 ["stamina"] = .5,
						 ["ammo"] = {},
						 ["health"] = 110,
						 ["armor"] = 0,
						 ["recoilmultiplier"] = 6,
						 ["walkspeed"] = 1,
						 ["runspeed"] = 1,
						 ["jumppower"] = 1,
						 ["genders"] = {true, false},
						 ["hairf"] = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 ["models"] = {"models/cultist/humans/sci/scientist.mdl"},
						 ["fmodels"] = {"models/cultist/humans/sci/scientist_female.mdl"},
						 ["showmodel"] = "models/cultist/humans/sci/scientist.mdl",
						 ["level"] = 25,
						 ["max"] = 1,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},*/
						{name = ROLES.ROLE_SCI_MEDIC,
						 team = TEAM_SCI,
						 weapons = {"br_holster", "weapon_pass_medic", "item_medkit_2", "item_deffib_medic"},		
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headf = {"models/cultist/heads/female/head_1.mdl"},
						 tasks = "Побег из комплекса, лечение союзников",
						 bodygroup1 = 1,
						 bodygroup4 = 1,
						 bodygroup5 = 1,
						 keycard = "sci_2",
						 stamina = 1.1,
						 ammo = {},
						 health = 100,
						 recoilmultiplier = 6,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 genders = {true, false},
						 hairf = {"models/cultist/heads/female/hair/hair_1.mdl"},
						 models = {"models/cultist/humans/sci/scientist.mdl"},
						 fmodels = {"models/cultist/humans/sci/scientist_female.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist.mdl",
						 level = 5,
						 max = 1,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
						 },
						},
						{name = ROLES.ROLE_SCI_HEAD,
						 team = TEAM_SCI,
						 weapons = {"br_holster", "weapon_pass_sci"},
						 headgear = "models/shaky/heads/head_gp.mdl",
						 tasks = "Побег из комплекса, координация Научным Отделом",
						 keycard = "sci_4",
						 ammo = {},
						 health = 115,
						 stamina = .7,
						 armor = 0,
						 recoilmultiplier = 6,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/sci/scientist_boss.mdl"},
						 showmodel = "models/cultist/humans/sci/scientist_boss.mdl",
						 level = 34,
						 max = 1,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 0.8,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 0.8,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
								   
						},
						
				}
			}


		       },
				
					
				security = {
					name = "Служба Безопасности",
					color = Color(123, 104, 238),
					roles = {
						{name = ROLES.ROLE_SECURITYRECRUIT,
						team = TEAM_SECURITY,
						weapons = {"br_holster", "weapon_pass_guard", "item_radio", "cw_kk_ins2_g17", "weapon_cqc", "item_tazer"},
						head = "models/cultist/humans/balaclavas/balaclava_full.mdl",
						headgear = "models/cultist/humans/security/head_gear/helmet_cap.mdl",
						tasks = "Побег из комплекса, защита учёных",
						bodygroups = "2185215",
						 keycard = "security_1",
						 ammo = {},
						 white = true,
						 health = 120,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 0,
						 max = 5,
						 damage_modifiers = {
							["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1					 
							},
						},
						{name = ROLES.ROLE_SECURITYSERG,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_m45", "weapon_cqc", "weapon_pass_guard"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet_cap.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 bodygroup0 = 1,
						 bodygroup3 = 3,
						 bodygroup1 = 1,
						 white = true,
						 keycard = "security_3",
						 ammo = {},
						 health = 150,
						 armor = 0,
						 bodyresist = 3,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 9,
						 max = 1,
						 bodygroups = "021014",
						 damage_modifiers = {
							["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
							},
						},
						{name = ROLES.ROLE_SECURITYOFFICER,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_g18", "weapon_cqc", "item_tazer", "weapon_pass_guard"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 bodygroup0 = 1,
						 bodygroup1 = 1,
						 white = true,
						 bodygroup3 = 3,
						 bodyresist = 3,
						 keycard = "security_2",
						 ammo = {},
						 health = 150,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 bodygroups = "020104001",
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 2,
						 max = 4
						},
						{name = ROLES.ROLE_SECURITYSPY,
						 team = TEAM_CHAOS,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_g18", "weapon_cqc", "item_tazer", "weapon_pass_guard"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 tasks = "Побег из комплекса, помощь Классу-Д",
						 ability = {"Focus", 40, "Показывает вам всех сотрудников Класса-Д", "nextoren/gui/special_abilities/spy_chaos_focus.png", false},
						 stamina = 1.5,
						 white = true,
						 bodyresist = 1,
						 headresist = 1,
						 keycard = "security_1",
						 ammo = {},
						 health = 120,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 bodygroups = "020104001",
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 21,
						 max = 1
						},
						{name = ROLES.ROLE_SECURITYSTORMTROOPER,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_mp5k", "weapon_cqc", "weapon_pass_guard"},
						 head = "models/cultist/humans/balaclavas/balaclava_full.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet_glass.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 bodygroups = "131003034",
						 headresist = 3,
						 white = true,
						 keycard = "security_2",
						 ammo = {},
						 health = 150,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 14,
						 max = 1,
						 damage_modifiers = {
							 ["HITGROUP_HEAD"] = 1,
							 ["HITGROUP_CHEST"] = 1,
							 ["HITGROUP_LEFTARM"] = 1,
							 ["HITGROUP_RIGHTARM"] = 1,
							 ["HITGROUP_STOMACH"] = 1,
							 ["HITGROUP_GEAR"] = 1,
							 ["HITGROUP_LEFTLEG"] = 1,
							 ["HITGROUP_RIGHTLEG"] = 1
							},
						},
						{name = ROLES.ROLE_SECURITYWARDEN,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_cobra", "weapon_cqc", "weapon_pass_guard", "item_tazer"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet_glass.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 bodygroup2 = 1,
						 bodygroup3 = 2,
						 bodygroup5 = 1,
						 headresist = 3,
						 white = true,
						 bodygroup6 = 1,
						 keycard = "security_3",
						 ammo = {},
						 health = 130,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 3,
						 bodygroups = "030112011",
						 max = 1
						},
						{name = ROLES.ROLE_SECURITYIMV,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_g19", "weapon_cqc", "weapon_pass_guard", "item_tazer"},
						 head = "models/cultist/humans/balaclavas/balaclava_full.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet_glass.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 bodygroup2 = 1,
						 bodygroup3 = 2,
						 bodygroup5 = 1,
						 headresist = 2,
						 white = true,
						 bodygroup6 = 1,
						 keycard = "security_2",
						 ammo = {},
						 health = 120,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 26,
						 bodygroups = "020005011",
						 max = 3
						},
						{name = ROLES.ROLE_SECURITYCHIEF,
						 team = TEAM_SECURITY,
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_deagle", "weapon_cqc", "weapon_pass_guard"},
						 head = "models/cultist/heads/male/head_main_1.mdl",
						 headgear = "models/cultist/humans/security/head_gear/helmet_beret.mdl",
						 tasks = "Побег из комплекса, защита учёных",
						 keycard = "security_4",
						 stamina = 1.1,
						 white = true,
						 bodyresist = 2,
						 ammo = {},
						 health = 130,
						 armor = 0,
						 walkspeed = 1,
						 runspeed = 1,
						 jumppower = 1,
						 models = {"models/cultist/humans/security/security.mdl"},
						 showmodel = "models/cultist/humans/security/security.mdl",
						 level = 34,
						 bodygroups = "042110001",
						 max = 1
						}
			
					}
			
				},
				mtf = {
			
					name = "Охрана",
			
					color = Color(0, 100, 255),
			
					roles = {
			
						{name = ROLES.ROLE_MTFGUARD,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cq300", "gasmask", "item_nightvision_green", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Ключ карта 3", "Радио", "MP7",},
	
						 keycard = "guard_2",
			
						 health = 150,
			
						 armor = 30,
				
						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 0,
			
						 max = 6,
			
						 sorting = 0,
			
						 bodygroups = "1110000001",
		
						 importancelevel = 1
			
						},
			
						{name = ROLES.ROLE_MTFMEDIC,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_famas", "item_medkit_4", "weapon_pass_guard", "item_deffib_medic", "item_syringe"},
			
						 showweapons = {"Ключ карта 3", "Радио", "Medkit", "famas"},
				
						 health = 150,
			
						 armor = 0,
						
						 runspeed = 1.1,
			
						 keycard = "guard_2",
			
						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 1,
			
						 max = 2,
			
						 sorting = 3,
			
						 bodygroups = "1110000001",
	
						 skin = 1,
			
						 importancelevel = 1
			
						},
			
						{name = ROLES.ROLE_MTFL,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_hk416c", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Kлюч-карта командира", "Рация", "MP5K"},
			
						 health = 165,
			
						 armor = 0,

						 stamina = 1.4,
			
						 walkspeed = 0.87,
			
						 runspeed = 0.94,
			
						 jumppower = 0.9,
			
						 vest = nil,
	
						 keycard = "guard_4",
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 1,
			
						 max = 1,
			
						 sorting = 4,
		
						 bodygroups = "01211002",
			
						 importancelevel = 1
			
						},
						{name = ROLES.ROLE_MTFCOM,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_uar556", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Ключ-карта командира", "Рация", "SCAR"},
				
						 health = 165,
			
						 armor = 50,
						
						 runspeed = 1.1,
						
						 stamina = 1.4,
	
						 keycard = "guard_4",
			
						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 4,
			
						 max = 1,
			
						 sorting = 6,
		
						 bodygroups = "31201",
			
						 importancelevel = 3
			
						},
			
						{name = ROLES.ROLE_MTFSHOCK,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_ksg", "item_nightvision_green", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Ключ карта МОГа", "Рация", "Дробовик KSG"},
		
						 health = 150,
			
						 armor = 20,
	
						 keycard = "guard_2",

						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 8,
			
						 max = 1,
			
						 sorting = 4,
			
						 importancelevel = 3,
		
						 bodygroups = "122011110"
			
						},
			
						{name = ROLES.ROLE_Engi,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_m40a1", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Ключ карта МОГа", "Рация", "Установка турели"},
			
						 health = 130,
						
						 walkspeed = 0.9,
			
						 runspeed = 1.1,
			
						 jumppower = 0.9,
	
						 keycard = "guard_3",
			
						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 7,
			
						 max = 1,
			
						 sorting = 4,
		
						 ability = {"Anti-SCP Turret", 1, "Вы ставите турель.", "nextoren/gui/special_abilities/engineer_turret.png", true},
		
						 ability_max = 1,
		
						 bodygroups = "4131000001",
			
						 importancelevel = 3
			
						},
			
						{name = ROLES.ROLE_MTFCHEMIST,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_kriss", "weapon_pass_guard", "item_syringe", "item_adrenaline", "item_pills", "item_medkit_3"},
			
						 showweapons = {"Ключ карта охраны", "Радио", "Camera View", "G36C"},
				
						 health = 120,
						
    					 runspeed = 1.1,
			
						 jumppower = 0.92,
	
						 keycard = "guard_3",
			
						 vest = nil,
			
						 models = CHLENIST,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 15,
			
						 max = 1,
			
						 sorting = 1,
		
						 bodygroups = "01",
			
						 importancelevel = 1
			
						},
			
						{name = ROLES.ROLE_SPECIALIST,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_blackout", "item_nightvision_blue", "item_medkit_3", "weapon_pass_guard"},
			
						 showweapons = {"Ключ карта 3", "Радио", "Medkit", "ACR", "Химические гранаты"},
				
						 health = 190,
			
						 armor = 70,
			
						 walkspeed = 1.05,
			
						 runspeed = 1.07,
			
						 jumppower = 0.87,
	
						 keycard = "guard_3",
			
						 vest = nil,
			
						 models = MTFMODEL,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 18,
			
						 max = 1,
			
						 sorting = 3,
		
						 bodygroups = "21201",
			
						 importancelevel = 1
			
						},
			
						{name = ROLES.ROLE_MTFJAG,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_m249", "item_medkit_3", "weapon_pass_guard", "item_drink_coffe", "item_adrenaline"},
			
						 showweapons = {"Ключ карта 3", "Радио", "M249"},
			
						 health = 200,
			
						 armor = 125,

						 stamina = .6,
			
						 walkspeed = 0.8,
			
						 runspeed = 0.8,
			
						 jumppower = 0.8,
			
						 vest = nil,
			
						 models = JAG,
			
						 showmodel = "models/cultist/humans/mog/mog_jagger.mdl",
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 31,
			
						 max = 1,
			
						 sorting = 1,
	
						 keycard = "guard_3",
			
						 importancelevel = 3
			
						},
			
						{name = ROLES.ROLE_HOF,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_revolver", "item_medkit_2", "weapon_pass_guard"},
	
						 keycard = "6",
			
						 showweapons = {"Ключ карта 5", "Радио"},
			
						 health = 100,
						
						 walkspeed = 1,
			
						 runspeed = 1.1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = GK,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 37,
			
						 max = 1,
			
						 sorting = 6,
			
						 importancelevel = 3
			
						}
			
					}
			
				},
				support = {
			
					name = "Поддержка",
			
					color = Color(29, 81, 56),
			
					roles = {
			
						{name = ROLES.ROLE_MTFNTF,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_p90", "item_nightvision_green", "weapon_pass_guard"},
			
						 showweapons = {"Командирская КЛЮЧ-КАРТА", "Радио", "P90", "Граната", "ПНВ"},
			
						 ammo = {{"AR2", 1200}},
			
						 health = 200,
		
						 keycard = "support",
	
						 armor = 50,
			
						 walkspeed = 0.86,
			
						 runspeed = 0.93,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = NTFMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 0,
		
						 bodygroups = "1011111111",
			
						 max = 3
			
						},
			
						{name = ROLES.ROLE_NTFCMD,
			
						 team = TEAM_GUARD,
			
						 weapons = {"br_holster", "item_radio", "item_nvg", "cw_kk_ins2_p90", "cw_kk_ins2_deagle", "item_nightvision_blue", "weapon_pass_guard", "item_drink_soda"},
			
						 showweapons = {"Командирская КЛЮЧ-КАРТА", "Радио", "P90", "Граната", "ПНВ"},
			
						 ammo = {{"AR2", 600}, {"Revolver", 7*6}},
			
						 health = 180,
	
						 keycard = "support",
			
			
			
						 armor = 35,
			
						 walkspeed = 0.86,
			
						 runspeed = 0.93,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = NTFMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 16,
		
						 ability = {"Cameras Scan", 120, "None Provided", "nextoren/gui/special_abilities/special_ntf_commander.png", false},
		
						 bodygroups = "1021110111",
			
						 max = 1
			
						},
			
			
			
						{name = ROLES.ROLE_CHAOS,
			
						 team = TEAM_CHAOS,
			
						 weapons = {"br_holster", "cw_kk_ins2_ak12", "item_radio", "item_syringe",},
			
						 showweapons = {"Ключ карта 5", "Радио", "RPK", "С4"},
			
						 ammo = {{"ar2", 560}},
			
						 health = 150,
			
						 armor = 80,
			
						walkspeed = 0.86,
			
				
						keycard = "support",
			
						 runspeed = 0.88,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = CHAOSMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 0,
		
						 bodygroups = "0101111",
			
						 max = 3
			
						},
			
						
			
						{name = ROLES.ROLE_CHAOSCMD,
			
						 team = TEAM_CHAOS,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_scar", "item_syringe",},
			
						 showweapons = {"Ключ карта 5", "Радио", "RPK", "С4"},
			
						 ammo = {{"ar2", 560}, {"PG-7VM Grenade", 2}},
			
						 health = 150,
			
						 armor = 100,
			
						 walkspeed = 0.86,
	
						 keycard = "support",
				
						 runspeed = 0.88,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = CHAOSMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 20,
		
						 bodygroups = "00011201",
			
						 max = 1
			
						},
			
				  {name = ROLES.ROLE_CHAOSDESTROYER,
			
						 team = TEAM_CHAOS,
			
						 weapons = {"br_holster", "cw_kk_ins2_cstm_spas12", "cw_kk_ins2_nade_molotov", "item_radio", "weapon_flashlight", "cw_kk_ins2_rpg"},
			
						 showweapons = {"Ключ карта 5", "RPG", "C4", "Дробовик"},
			
						 ammo = {{"Shotgun", 560}, {"RPG_Rocket", 2}},
			
						 health = 170,
			
						 armor = 80,
			
						 walkspeed = 0.86,
	
						 keycard = "support",
				
						 runspeed = 0.91,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = CHAOSMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 0,
		
						 ability = {"Claymore", 1, "Вы ставите мину на пол.", "nextoren/gui/special_abilities/special_chaos_claymore.png", true},
		
						 ability_max = 3,
		
						 bodygroups = "1002001",
			
						 max = 1
			
						},
						{name = ROLES.ROLE_CHAOSJUGGERNAUT,
			
						team = TEAM_CHAOS,
		   
						weapons = {"br_holster", "item_radio", "cw_cultist_machinegun_m60", "item_syringe"},
		   
						showweapons = {""},
		   
						ammo = {{"ar2", 600}},
		   
						health = 220,
		   
						armor = 150,
	
						keycard = "support",
		   
						walkspeed = 0.86,
			   
						runspeed = 0.93,
		   
						jumppower = 0.85,
		   
						vest = nil,
		   
						models = CHAOSMODELS,
		   
						showmodel = nil,
		   
						pmcolor = nil,
	
						keycard = "support",
		   
						flashlight = true,
		   
						level = 0,
		
						bodygroups = "20031101",
		   
						max = 1
		   
					   },
			
						{name = ROLES.ROLE_GoP,
			
						 team = TEAM_GOC,
			
						 weapons = {"br_holster", "item_radio", "cw_cultist_semisniper_arx160", "item_nightvision_goc"},
			
						 showweapons = {"Ключ-карта ГОКа", "Рация", "Дубинка", "Скар и сканнер"},
			
						 ammo = {{"GOC", 600}},
			
						 health = 150,
			
						 armor = 80,
			
						 walkspeed = 0.86,
			
						 runspeed = 0.93,
	
						 keycard = "support",
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = GOCMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 5,
							 
						 max = 3
			
						},
						{name = ROLES.ROLE_GOC_JAG,
			
						team = TEAM_GOC,
		   
						weapons = {"br_holster", "item_radio", "cw_cultist_goc_fate", "item_nightvision_goc", "item_adrenaline"},
		   
						showweapons = {""},
		
						ammo = {{"GOC", 600}},
		
						ability = {"Shield", 75, "Активируйте щит что-бы защитить себя и своих товарищей.", "nextoren/gui/special_abilities/special_goc_shield.png", false},
		   
						health = 220,
		   
						armor = 100,
	
						keycard = "support",
		   
						walkspeed = 0.86,
		   
						runspeed = 0.93,
		   
						jumppower = 0.85,
		   
						vest = nil,
		   
						models = GOCMODELS,
		   
						showmodel = nil,
		   
						pmcolor = nil,
		   
						flashlight = true,
		   
						level = 5,
						
						bodygroups = "12",
							
						max = 1
		   
					   },
			
						{name = ROLES.ROLE_GoPCMD,
			
						 team = TEAM_GOC,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_nv4", "item_nightvision_goc"},
			
						 showweapons = {"Ключ-карта ГОКа", "Рация", "Дубинка", "Скар и сканнер"},
			
						 ammo = {{"GOC", 410}},
			
						 health = 155,
			
						 armor = 131,
			
						 walkspeed = 0.86,
	
						 keycard = "support",
			
						 runspeed = 0.93,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = GOCMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 14,
		
						 bodygroups = "33",
		
						 ability = {"Cloak", 80, "None Provided", "nextoren/gui/special_abilities/special_goc_invisibility.png", false},
			
						 max = 1
			
						},
			
						{name = ROLES.ROLE_USA,
			
						 team = TEAM_USA,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_m16a4", "item_nightvision_blue"},
			
						 showweapons = {"Ключ-карта КПП", "Рация"},
			
						 ammo = {{"AR2", 800}},
			
						 keycard = "support",
			
						 health = 150,
			
						 armor = 90,
			
						 walkspeed = 1.05,
			
						 runspeed = 1.10,
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = USAMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 5,
		
						 bodygroups = "1001",
			
						 max = 3
			
						},
						{name = ROLES.ROLE_UIU_Clocker,
			
						team = TEAM_USA,
		   
						weapons = {"br_holster", "item_radio", "cw_fbi_revolver", "weapon_fbi_knife", "item_nightvision_blue"},
		   
						showweapons = {""},
		   
						ammo = {{"Revolver", 48}},
		   
		   
						health = 200,
		   
						armor = 90,
			 
						keycard = "support",
						walkspeed = 0.86,
						runspeed = 1.2,
						jumppower = 0.85,
		   
						vest = nil,
		   
						models = USAMODELS,
		   
						showmodel = nil,
		   
						pmcolor = nil,
		   
						flashlight = true,
		   
						level = 5,
		
						bodygroups = "3100",
		
						ability = {"Rage", 75, "Increases your movement speed by a minute", "nextoren/gui/special_abilities/special_fbi_clocker.png", false},
		
						damage_modifiers = {
							["HITGROUP_HEAD"] = 0.7,
							["HITGROUP_CHEST"] = 0.6,
							["HITGROUP_LEFTARM"] = 0.8,
							["HITGROUP_RIGHTARM"] = 0.8,
							["HITGROUP_STOMACH"] = 0.6,
							["HITGROUP_GEAR"] = 0.6,
							["HITGROUP_LEFTLEG"] = 0.8,
							["HITGROUP_RIGHTLEG"] = 0.8
						},
		
		   
						max = 3
		   
					   },
			
						{name = ROLES.ROLE_USACMD,
			
						 team = TEAM_USA,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_m16a4", "item_nightvision_blue"},
			
						 showweapons = {"Ключ-карта КПП", "Рация"},
			
						 ammo = {{"ar2", 800}},
			
			
			
						 health = 165,
			
						 armor = 35,
			
						 walkspeed = 1.05,
			
						 runspeed = 1.25,
			
						 jumppower = 0.85,
	
						 keycard = "support",
			
						 vest = nil,
			
						 models = USAMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 7,
		
						 bodygroups = "2001",
		
						 ability = {"Intelligence", 45, "None Provided", "nextoren/gui/special_abilities/special_fbi_commander.png", false},
			
						 max = 1
			
						},
			
				  {name = ROLES.ROLE_DZ,
			
						 team = TEAM_DZ,
			
						 weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_galil_ace"},
			
						 showweapons = {"???", "Ключ-карта ???", "Информация недоступна", "████████████"},
			
						 ammo = {{"SMG1", 800}},
			
						 tasks = "Спасение SCP-объектов, огневая поддержка SCP",
			
						 health = 145,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 0.93,
	
						 keycard = "support",
			
						 jumppower = 0.85,
			
						 vest = nil,
			
						 models = DZMODELS,
			
						 showmodel = nil,
			
						 pmcolor = nil,
			
						 flashlight = true,
			
						 level = 5,
		
						 bodygroups = "300001",
			
						 max = 4
			
						},
						{name = ROLES.ROLE_DZCMD,
			
						team = TEAM_DZ,
		   
						weapons = {"br_holster", "item_radio", "cw_kk_ins2_cstm_g36c"},
		   
						showweapons = {"???", "Ключ-карта ???", "Информация недоступна", "████████████"},
	
						tasks = "Спасение SCP-объектов, огневая поддержка SCP",
		   
						ammo = {{"ar2", 800}},
		   
						keycard = "support",
			
						health = 150,
		   
						armor = 0,
		   
						walkspeed = 1,
		   
						runspeed = 0.93,
		   
						jumppower = 0.85,
		   
						vest = nil,
		   
						models = DZMODELS,
		   
						showmodel = nil,
		   
						pmcolor = nil,
		   
						flashlight = true,
		   
						level = 7,
		
						ability = {"Portal", 45, "Вы ставите портал который случайно телепортирует вас в одну из зон комплекса", "nextoren/gui/special_abilities/special_dz_portal.png", false},
		
						bodygroups = "132110",
		   
						max = 1
		   
					}
			
				}
		
			},
				special = {
			
					name = "Сюжетные ученые",
			
					color = Color(238, 130, 238),
			
					roles = {
			
					  {name = ROLES.ROLE_SPECIALRES,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3", "item_nvg"},
			
						 showweapons = {"Специальная способность - Исцеление по области"},
			
						 ammo = {{"Pistol", 20}},
				
						 health = 120,
			
						 armor = 0,
			
						 walkspeed = 0.85,
			
						 runspeed = 0.90,
			
						 jumppower = 1,
			
						 vest = nil,
	
						 keycard = "sci_3",
			
						 models = SPECIAL4,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 0,
		
						 ability = {"AOE Heal", 75, "Исцеляет людей в некотором радиусе от вас.", "nextoren/gui/special_abilities/special_heal.png", false},
			
						 max = 1
			
						},
			
			
			
						{name = ROLES.ROLE_SPECIALRESS,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3"},
			
						 showweapons = {"Специальная способность - Обнаружение всех живых СЦП объектов в течении 10 секунд"},
			
						 ammo = {{"Pistol", 20}},
			
						 health = 110,
			
						 armor = 0,
			
						 walkspeed = 0.91,
			
						 runspeed = 0.95,
			
						 jumppower = 1,
	
						 keycard = "sci_3",
			
						 vest = nil,
			
						 models = SPECIAL2,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 0,
		
						 ability = {"Special Vision", 60, "Вы начинаете видеть SCP через стены.", "nextoren/gui/special_abilities/spy_chaos_focus.png", false},
			
						 max = 1
			
						},
			
						
			
						{name = ROLES.ROLE_SPECIALRESSSS,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3", "item_nvg"},
			
						 showweapons = {"Специальная способность",  "Замедление SCP-Объектов"},
			
						 ammo = {{"Pistol", 20}},
			
			
			
						 health = 150,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 1,
			
						 jumppower = 1,
	
						 keycard = "sci_3",
			
						 vest = nil,
			
						 models = SPECIAL3,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 8,
		
						 ability = {"Slow", 85, "Замедляет SCP в некотором радиусе от вас.", "nextoren/gui/special_abilities/special_slow.png", false},
			
						 max = 1
			
						},
			
						
			
						{name = ROLES.ROLE_SHIELD,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_hexshield_local", "weapon_flashlight", "br_keycard_3"},
			
						 showweapons = {"Специальная способность",  "Защитный купол"},
			
						 ammo = {{"Pistol", 20}},
			
						 keycard = "sci_3",
			
						 health = 210,
			
						 armor = 0,
			
						 walkspeed = 0.89,
			
						 runspeed = 0.94,
			
						 jumppower = 0.90,
			
						 vest = nil,
			
						 models = SPECIAL7,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 24,
		
						 ability = {"Special Shield", 80, "Вы создаёте вокруг себя щит, не пропускающий SCP-объекты.", "nextoren/gui/special_abilities/special_shield.png", false},
			
						 max = 1
			
				 },
			
				 
			
						{name = ROLES.ROLE_SPEEED,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3", "item_nvg"},
			
						 showweapons = {"Специальная способность",  "Ускорение"},
			
						 ammo = {{"Pistol", 20}},
			
						 keycard = "sci_3",
			
						 health = 120,
			
						 armor = 0,
			
						 walkspeed = 1.1,
			
						 runspeed = 1.2,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = SPECIAL1,
			
						 showmodel = nil,
			
						 flashlight = false,
						
						 ability = {"Speed Boost", 45, "Вы делаете себя и людей в радиусе быстрыми.", "nextoren/gui/special_abilities/special_sprint.png", false},
						 
						 level = 10,
			
						 max = 1
			
						},
			
						
			
						{name = ROLES.ROLE_LESSION,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3", "item_nvg"},
			
						 showweapons = {"Специальная способность",  "Мины XCX"},
			
						 ammo = {{"Pistol", 20}},
			
						 keycard = "sci_3",
			
						 health = 130,
			
						 armor = 0,
			
						 walkspeed = 0.96,
			
						 runspeed = 1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = SPECIAL6,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 22,
		
						 ability = {"Anti-SCP mine", 3, "Вы ставите Anti-SCP мину.", "nextoren/gui/special_abilities/special_mine.png", true},
		
						 ability_max = 3,
			
						 max = 1
			
						},
			
						
			
						{name = ROLES.ROLE_SPECIALRESSS,
			
						 team = TEAM_SPECIAL,
			
						 weapons = {"br_holster", "weapon_flashlight", "br_keycard_3", "item_nvg"},
			
						 showweapons = {"Специальная способность - Увелечение пассивного урона по SCP-Объектам"},
			
						 ammo = {{"Pistol", 20}},
			
						 keycard = "sci_3",
			
						 health = 210,
			
						 armor = 0,
			
						 walkspeed = 1,
			
						 runspeed = 1.1,
			
						 jumppower = 1,
			
						 vest = nil,
			
						 models = SPECIAL5,
			
						 showmodel = nil,
			
						 flashlight = false,
			
						 level = 19,
		
						 ability = {"Damage Boost", 75, "Вы увеличиваете урон людям в радиусе, урон по SCP.", "nextoren/gui/special_abilities/special_debuffscp.png", false},
			
						 max = 1
						}
			
					}
			
				}
			
			}
			
			
			