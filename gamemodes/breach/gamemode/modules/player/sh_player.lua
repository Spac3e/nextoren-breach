...
hook.Add( "PlayerButtonDown", "Specials", function( ply, button )
	...
	elseif ply:HaveSpecialAb(role.ClassD_Thief) then
		...
		ply:BrProgressBar("l:stealing", 1.45, "nextoren/gui/special_abilities/ability_placeholder.png", target, false, function()
			if IsValid(stealweapon) and stealweapon:GetOwner() == target and ply:GTeam() != TEAM_SPEC then
				ply:BreachGive(stealweapon:GetClass())
				target:StripWeapon(stealweapon:GetClass())
				target:SelectWeapon("br_holster")
				ply:SetSpecialCD(CurTime() + 45)
			end
		end)
		...
		elseif ply:HaveSpecialAb(role.NTF_Specialist) then
			...
			if target && target:IsValid() && target:IsPlayer() && target:GTeam() == TEAM_SCP && target:Health() > 0 && target:Alive() then
				...
				if target:GetModel() == "models/cultist/scp/scp_682.mdl" then
					target:SetForcedAnimation("0_Stun_29", 6)
				else
					target:SetForcedAnimation("0_SCP_542_lifedrain", 6)
				end
...
if SERVER then
		...
		elseif ply:HaveSpecialAb(role.SCI_SPECIAL_VISION) then
			...
			if SERVER then ply:EmitSound("nextoren/vo/special_sci/pulse/pulse_"..math.random(1,14)..".mp3") end
		...
		elseif ply:HaveSpecialAb(role.SCI_SPECIAL_HEALER) then
			...
			BroadcastLua("ParticleEffectAttach(\"mr_effect_03_overflow\", PATTACH_POINT_FOLLOW, Entity("..ply:EntIndex().."), Entity("..ply:EntIndex().."):LookupAttachment(\"chest\"))")

			timer.Create("megahealeffect"..ply:SteamID64(), 6, 1, function()
				ply:StopParticles()
			end)
