...
function ENT:Initialize()
    ...
    if IsValid(self) then
		self:SetSequence(self:LookupSequence("rotating"))
		self:SetPlaybackRate(5)
	
        timer.Create( "1_phase", 19.5, 1, function()
			self:ResetSequence(5)
            self:StopSound("nextoren/others/helicopter/helicopter_propeller.wav")
            self:EmitSound( "nextoren/vo/chopper/chopper_evacuate_start_7.wav") 

			self:SetSequence(self:LookupSequence("door_open"))
			self:SetPlaybackRate(0.1)

			self:ResetSequence(self:LookupSequence("door_open"))

			self:SetSequence(self:LookupSequence("door_opened"))
			
			timer.Simple(00000.1, function()
			for k,v in pairs(ents.FindByClass("heli")) do v:SetAngles(Angle(0,90,0)) end
			end)
        ...
        timer.Create( "4_phase", 90, 1, function()
            self:SetSequence(self:LookupSequence("door_close"))
			self:SetPlaybackRate(0.1)
			self:ResetSequence(self:LookupSequence("door_close"))

			self:SetSequence(self:LookupSequence("door_closed"))
			self:ResetSequence(self:LookupSequence("door_close"))
...
function ENT:Think()
    if timer.Exists("1_phase") then
        self.SequenceID = self:LookupSequence("sequence_name_3") -- Sequence 3
    elseif timer.Exists("4_phase") then
        self.SequenceID = self:LookupSequence("sequence_name_2") -- Sequence 2
    end

   -- self:AnimateBlades() -- Обновляем анимацию лопастей
