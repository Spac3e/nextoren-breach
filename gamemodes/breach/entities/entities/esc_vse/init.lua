...
-- Побег через машину
	for k,v in pairs(ents.FindInSphere(Vector(-7112, 15128, 2153), 100)) do
		...
		evacuate(v,"vse",900,"l:ending_escaped_site19")
...
-- Побег через ОНП
	for k,v in pairs(ents.FindInSphere(Vector(-7024, -960, 1793), 5)) do
		evacuate(v,"vse",400,"l:ending_escaped_site19")
	end
-- Побег через руку
	for k,v in pairs(ents.FindInSphere(Vector(-8001, -810, 1804), 50)) do
		evacuate(v,"vse",750,"l:ending_escaped_site19")
	end
-- Побег через РПХ
	for k,v in pairs(ents.FindInSphere(Vector(-1969, 4863, 1515), 50)) do
		evacuate(v,TEAM_CLASSD,2300,"l:ending_escaped_site19")
	end
