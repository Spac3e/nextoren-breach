...
hook.Add("PreDrawOutlines", "CHAOS_SPY", function()
  ...
  if client_team == TEAM_CHAOS or client_team == TEAM_CLASSD then
	  for i = 1, #plys do
	  	...
	  	if ply:GetNClass() != ROLES.ROLE_CHAOSSPY then continue end
...
net.Receive("NTF_Intro", function()
	...
	timer.Simple(1, function()
		...
		ntf_helicopter:SetPos(Vector(-10600.646484, -77.443832, 2373.897461)) 
	...
	*/
	...
	if shoulddrawinfo == true then
		if LocalPlayer():GTeam() == TEAM_SPEC then
			...
			local client = LocalPlayer()
			if (client:GTeam() != TEAM_QRT and client:GTeam() != TEAM_GUARD and ( client:GTeam() != TEAM_DZ or client:GetNClass() == ROLES.ROLE_DZDD ) and client:GTeam() != TEAM_NTF and ( client:GTeam() != TEAM_USA or client:GetNClass() == ROLES.ROLE_USASPY ) and client:GTeam() != TEAM_GRU and ( client:GTeam() != TEAM_GOC or client:GetNClass() == ROLES.ROLE_GOCSPY ) and client:GTeam() != TEAM_COTSK and ( client:GTeam() != TEAM_CHAOS or client:GetNClass() == ROLES.ROLE_CHAOSSPY )) or client:GetNClass() == ROLES.ROLE_NTFPILOT then
				DrawNewRoleDesc()
			end
