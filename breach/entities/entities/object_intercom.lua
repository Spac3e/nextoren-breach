...
if SERVER then
	net.Receive("IntercomAction", function(len, ply)
		if ply:GetNClass() != role.Dispatcher then return end
...
if CLIENT then
	net.Receive("OpenIntercomMenu", function()
		--if activator:GetNClass() == role.MTF_HOF or activator:GetNClass() == role.Dispatcher then
				...
				BREACH.DispatchSpecial.MainPanel.Disclaimer.Paint = function( self, w, h )
					...
					if ( client:GetNClass() != role.Dispatcher || client:Health() <= 0 ) then
...
function ENT:Use(activator, caller)
	if activator:GetNClass() == role.Dispatcher and self.DispatchCD <= CurTime() then
...
function ENT:Draw()
	...
	cam.Start3D2D(self:LocalToWorld(Vector(6, 3, -10)), self:LocalToWorldAngles( offset_angle ), 0.25);
		...
		elseif ( self:GetStatus() == "Scientist" ) then
			local amount = 0
			for i, v in pairs(player.GetAll()) do
				if IsValid(v) and ( v:GTeam() == TEAM_SCI or v:GTeam() == TEAM_SPECIAL or v:GetModel():find("/sci/") ) and !v:GetModel():find("hazmat") and v:GetNClass() != role.Dispatcher and v:IsEntrance() then
					amount = amount + 1
				end
			end
