	...
	if ply:GetMaxSlots() + self.Slots <= 12 then 
		if SERVER then
			...
			if ply:GetUsingBag() != "" then
				ply:BrTip(0, "[RX Breach]", Color(255, 0, 0), "На вас уже надета сумка!", Color(255, 255, 255))
				return
			end

			ply:BrProgressBar("Подождите...", 1.5, "nextoren/gui/icons/hand.png", self, false, function()
				ply:BrTip(0, "[RX Breach]", Color(255, 0, 0), "Вы успешно надели сумку!", Color(255, 255, 255))
		...
		if SERVER then
			ply:BrTip(0, "[RX Breach]", Color(255, 0, 0), "Вы не можете надеть сумку!", Color(255, 255, 255) )
		end
	end
end
