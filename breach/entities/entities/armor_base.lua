	...
	if ( ply:GTeam() == TEAM_CLASSD or ply:GTeam() == TEAM_SCI or ply:GetNClass() == ROLES.ROLE_GOCSPY or ply:GetNClass() == ROLES.ROLE_USASPY or ply:GetNClass() == ROLES.ROLE_DZDZ ) and ply:GetNClass() != ROLES.ROLE_FAT and ply:GetNClass() != ROLES.ROLE_TOPKEK then
		...
		if SERVER then
			if string.len(ply:GetUsingCloth()) > 0 then
				ply:BrTip( 0, "[RX Breach]", Color(255, 0, 0), "На вас уже надета форма. Снимите её текущую форму.", Color(255, 255, 255) )
				return
			end
			if self:GetClass() != "armor_sci" and self:GetClass() != "armor_medic" then
				if ply:GetUsingArmor() != "" and ( self:GetClass() != "armor_sci" or self:GetClass() != "armor_medic" ) then
					ply:BrTip( 0, "[RX Breach]", Color(255, 0, 0), "Снимите броню, что-бы надеть униформу!", Color(255, 255, 255) )
					return
				end
			...
			ply:BrProgressBar("Подождите...", 7, "nextoren/gui/icons/hand.png", self, false, function()
				...
				ply:BrTip( 0, "[RX Breach]", Color( 0, 210, 0, 180 ), "Вы переоделись в "..self.PrintName, Color( 0, 255, 0, 180 ) )
				ply:SetupHands()
				if self.ArmorSkin then
