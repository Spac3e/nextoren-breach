...
File Path:   gamemodes/breach/entities/entities/onp_monitor.lua
      ...
      if ( SERVER ) then
        ...
        for i = 1, #players do
          ...
          if ( player:GTeam() == TEAM_USA ) then
            ...
            player:AddToStatistics( "Objective bonus", 40 )
        ...
        if ( monitors_left > 0 ) then
          ...
          BREACH.Players:ChatPrint( units, true, true, "Данные были получены. Осталось мониторов: " .. monitors_left )

        else

          m_UIUCanEscape = true
          BREACH.Players:ChatPrint( units, true, true, "Все необходимые данные были отправлены техническому отделу. Теперь Вы можете эвакуироваться." )
