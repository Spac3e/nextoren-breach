  ...
  for name, v in pairs( self.Stance_Animations[ stance_id ] ) do
    ...
    if ( !istable( v ) ) then
      ...
      if ( name:find( "walk" ) ) then
        ...
        self.Owner.SafeModeWalk = v

      elseif ( name:find( "idle" ) ) then

        self.Owner.IdleSafemode = v

      else

        self.Owner.SafeRun = v
...
if ( CLIENT ) then
  ...
  function SWEP:DrawWorldModel()
    ...
    if ( self:GetStance() == 1 && self.Owner.SafeModeWalk != 5309 ) then

      self:StanceChanged( 1, true )

    elseif ( self:GetStance() == 2 && self.Owner.SafeModeWalk != 5298 ) then
