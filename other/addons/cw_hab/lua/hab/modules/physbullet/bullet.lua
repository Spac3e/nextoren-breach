...
--		Ent:LagCompensation( false )
	...
	if prehit then -- we hit something
		...
		if ( chance > util.SharedRandom( "HAB_PhysBullet_SharedRandom", 0.0, 1.0, ( Bullet.RandomSeed + chance ) ) ) then -- chance to ricochet, calculate ricochet
...
hab.hook( "PreDrawEffects", "HAB_PhysBullet_PreDrawEffects", function( ) -- sprites
	...
	local enable_dlight = !tobool( hab.cval.PhysBullet.Disable_TracerLights )
	local enable_smoothing = !tobool( hab.cval.PhysBullet.Disable_TracerSmoothing )
	local enable_scaling = !tobool( hab.cval.PhysBullet.Disable_AdvancedTracerScaling )
	local enable_atcalc = !tobool( hab.cval.PhysBullet.Disable_AdvancedTracerCalc )
	local enable_glow = !tobool( hab.cval.PhysBullet.Disable_TracerGlow )
...
--[[
				...
				col.a = 248
				if enable_atcalc and v.TailPosition != v.LastPosition then
