...
File Path:   addons/[chat]_sh_lounge_chatbox/lua/autorun/cl_chatbox.lua
...
local render_character_avatar = CreateClientConVar("br_render_player_avatar", 1, true, false, 0, 1)
...
if !ply.Alive or !ply:Alive() then return end
    ...
    if (IsValid(ply)) then
        if ply:GTeam() != TEAM_SPEC then
            if render_character_avatar:GetBool() then
                ...
                if LocalPlayer():CanSeeEnt(ply) or ply == LocalPlayer() then
    ...
    if (darkrp) then
        ...
        if (IsValid(ply)) then
            if LOUNGE_CHAT.CustomTagsGroups[ply:GetUserGroup()] and ( ply:GTeam() == TEAM_SPEC ) then
            ...
            if ply:GTeam() == TEAM_SCP then
                ...
                Add({Color(255, 0, 0), "("..GetLangRole(ply:GetNClass())..")"})
